# MisterBot - Walk-forward predicción Ambos Marcan (BTTS)
...
df['ambos_marcaron_ult_jornada'] = ((df['goles_local'].shift(1) > 0) & (df['goles_visitante'].shift(1) > 0)).astype(int)
...
features = [col for col in df.columns if any(x in col for x in [
    ...
    'faltas_', 'tarjeta_', 'gol_', 'asistencia_', 'ambos_marcaron_ult_jornada'
])]
...
# ------------------------ #
df['target_btts'] = ((df['goles_local'] > 0) & (df['goles_visitante'] > 0)).astype(int)
resultados = []
...
for jornada in sorted(df['jornada'].unique()):
    ...
    df_test['pred_btts'] = pred
    df_test['proba_btts'] = proba
...
acc = accuracy_score(df_final['target_btts'], df_final['pred_btts'])
print(f"\n✅ Precisión global Ambos Marcan (walk-forward): {acc:.2%}")

print("\n📊 Clasificación:")
print(classification_report(df_final['target_btts'], df_final['pred_btts'], digits=4))

cm = confusion_matrix(df_final['target_btts'], df_final['pred_btts'], labels=[0, 1])
sns.heatmap(cm, annot=True, fmt='d', cmap='YlGnBu', xticklabels=['No BTTS', 'BTTS'], yticklabels=['No BTTS', 'BTTS'])
plt.title("Matriz de confusión final - Ambos Marcan")
...
# ------------------------ #
# Recomendaciones por alta confianza
# ------------------------ #
print("\n🎯 Partidos recomendados para apostar por BTTS (confianza >= 0.80):")
recomendados = df_final[(df_final['pred_btts'] == 1) & (df_final['proba_btts'] >= 0.8)]
print(recomendados[['jornada', 'equipo_local', 'equipo_visitante', 'proba_btts']].sort_values(by='proba_btts', ascending=False))
...
df_output['modelo'] = 'Neuronal_Ambos_Marcan'
df_output['prediccion'] = df_final['pred_btts'].astype(bool)
df_output['probabilidad'] = df_final['proba_btts']
...
# ------------------------ #
print("\n📈 Curva de calibración - BTTS")
y_true = df_final['target_btts']
y_proba = df_final['proba_btts']
...
plt.title('Curva de calibración - Ambos Marcan')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
